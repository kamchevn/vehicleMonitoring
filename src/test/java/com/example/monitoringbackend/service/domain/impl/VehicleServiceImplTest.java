...
class VehicleServiceImplTest {
    ...
    @Mock
    private com.example.monitoringbackend.service.domain.ComponentService componentService;

    @Mock
    private ComponentTemplateService componentTemplateService;

    @Mock
    private ComponentServiceRepository componentServiceRepository;

    @Mock
    private IntervalInsertRepository intervalInsertRepository;

    @Mock
    private InsertWindowStatusService insertWindowStatusService;
    ...
    void setUp() {
        ...
        vehicle = new Vehicle(
                "Civic", 2020, 50000, VehicleType.CAR, VehicleFuelType.PETROL,
                null, null, Condition.GOOD, IntervalInsertPeriod.WEEKLY, owner
        );
    ...
    void createNewVehicle_savesVehicleForUser() {
        ...
        Vehicle result = vehicleService.createNewVehicle(
                "Civic", 2020, 50000, "CAR", "PETROL", null, null,
                "GOOD", "WEEKLY", userDetails("alice", "ROLE_USER")
        );
    ...
    void editVehicle_swapsSparkPlugForGlowPlug_whenFuelTypeChangesToDiesel() {
        ...
        when(componentTemplateService.getTemplatesForVehicleTypeAndComponentType(VehicleType.CAR, ComponentType.GLOW_PLUG))
                .thenReturn(List.of(glowTemplate));
        when(componentService.save(any(Component.class))).thenAnswer(inv -> inv.getArgument(0));
        when(componentService.saveAll(any())).thenAnswer(inv -> inv.getArgument(0));
        when(vehicleRepository.save(any(Vehicle.class))).thenAnswer(inv -> inv.getArgument(0));

        Vehicle result = vehicleService.editVehicle(
                1L, "Civic", 2020, 50000, "CAR", "DIESEL", null, null,
                "GOOD", "WEEKLY", userDetails("alice", "ROLE_USER")
        );
