...
class ComponentServiceImplTest {
    ...
    void findAllByVehicleAndTemplateMeasuringUnitType_delegatesWithEnum() {
        when(componentRepository.findAllByVehicleAndTemplateMeasuringUnitType(vehicle, IntervalUnit.KILOMETERS))
                .thenReturn(List.of(component));

        List<Component> result = componentService.findAllByVehicleAndTemplateMeasuringUnitType(vehicle, "KILOMETERS");

        assertEquals(1, result.size());
        verify(componentRepository).findAllByVehicleAndTemplateMeasuringUnitType(vehicle, IntervalUnit.KILOMETERS);
    }
