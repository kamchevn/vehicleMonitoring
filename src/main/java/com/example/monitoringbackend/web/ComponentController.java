...
public class ComponentController {
    ...
    @GetMapping("/page")
    public ResponseEntity<Page<Component>> getComponentPage(
        ...
        Page<Component> page = this.componentService.findPage(vehicleId, measuringUnit, condition, Integer.parseInt(pageNum)-1, Integer.parseInt(pageSize));
    ...
    @PostMapping("/create")
    public ResponseEntity<Component> createVehicle(
            @RequestParam Long vehicleId,
            @RequestParam Long componentTemplateId,
            @RequestParam String condition,
            @RequestParam Integer counter){
        Vehicle vehicle = vehicleService.findById(vehicleId);
        return ResponseEntity.ok(componentService.createNewComponent(vehicle,componentTemplateId,condition,counter));
    }
    @PostMapping("/edit/{id}")
    public ResponseEntity<?> editVehicle(
        ...
        try {
            Vehicle vehicle = vehicleService.findById(vehicleId);
            return ResponseEntity.ok(componentService.editComponent(id,vehicle,componentTemplateId,condition,counter));
        } catch (VehicleNotFoundException ex) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ex.getMessage());
        }
    }
    ...
    public List<Component> findAllByVehicle(@PathVariable Long vehicleId) {
        Vehicle vehicle = vehicleService.findById(vehicleId);
        return componentService.findAllByVehicle(vehicle);
    }

    @GetMapping("/findByVehicleAndCondition/{vehicleId}/{condition}")
    public List<Component> findAllByVehicleAndCondition(@PathVariable Long vehicleId, @PathVariable String condition) {
