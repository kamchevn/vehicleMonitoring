...
public class VehicleServiceImpl implements VehicleService {
  ...
  public VehicleServiceImpl(
      ...
      com.example.monitoringbackend.service.domain.ComponentService componentService,
      ComponentTemplateService componentTemplateService,
      ComponentServiceRepository componentServiceRepository,
      IntervalInsertRepository intervalInsertRepository,
      InsertWindowStatusService insertWindowStatusService,
  ...
  public Vehicle findById(Long id) {
    ...
    vehicle.setInsertWindowStatus(
        insertWindowStatusService.calculate(vehicle, Clock.systemDefaultZone()));
    return vehicle;
  }

  @Override
  public Page<Vehicle> findPage(
    ...
    Page<Vehicle> vehicles =
        this.vehicleRepository.findAll(spec, PageRequest.of(pageNum, pageSize));
    vehicles
        .getContent()
        .forEach(
            vehicle ->
  ...
  public Vehicle createNewVehicle(
    ...
    return vehicleRepository.save(
        new Vehicle(name, year, totalKilometers, vt, ft, ct, dt, cdn, insertPeriod, userToSave));
  }
  ...
  public Vehicle editVehicle(
    ...
    if (oldFuelType != null && ft != null && !oldFuelType.equals(ft)) {
      ...
      if (oldFuelType == VehicleFuelType.PETROL && ft == VehicleFuelType.DIESEL) {
        ...
        ComponentTemplate glowPlugTemplate =
            componentTemplateService
                .getTemplatesForVehicleTypeAndComponentType(
        ...
        ComponentTemplate sparkPlugTemplate =
            componentTemplateService
                .getTemplatesForVehicleTypeAndComponentType(
                    VehicleType.CAR, ComponentType.SPARK_PLUG)
                .get(0);
    ...
    if (vt == VehicleType.MOTORCYCLE) {
      ...
      if (oldCoolingType != null && ct != null && !oldCoolingType.equals(ct)) {
        if (oldCoolingType == CoolingType.AIR_COOLED && ct == CoolingType.WATER_COOLED) {
          ComponentTemplate coolantTemplate =
              componentTemplateService
                  .getTemplatesForVehicleTypeAndComponentType(
                      VehicleType.MOTORCYCLE, ComponentType.COOLANT)
                  .get(0);
          newMotorcycleComponents.add(
              new Component(vehicle, coolantTemplate, Condition.UNKNOWN, 0));
      ...
      if (oldDrivenType != null && dt != null && !oldDrivenType.equals(dt)) {
        if (oldDrivenType == DrivenType.BELT_DRIVEN && dt == DrivenType.CHAIN_DRIVEN) {
          ...
          ComponentTemplate driveChainTemplate =
              componentTemplateService
                  .getTemplatesForVehicleTypeAndComponentType(
                      VehicleType.MOTORCYCLE, ComponentType.DRIVE_CHAIN)
                  .get(0);
          ComponentTemplate chainLubeTemplate =
              componentTemplateService
                  .getTemplatesForVehicleTypeAndComponentType(
                      VehicleType.MOTORCYCLE, ComponentType.CHAIN_LUBRICATION)
                  .get(0);
          ComponentTemplate sprocketsTemplate =
              componentTemplateService
                  .getTemplatesForVehicleTypeAndComponentType(
                      VehicleType.MOTORCYCLE, ComponentType.SPROCKETS)
                  .get(0);

          newMotorcycleComponents.add(
              new Component(vehicle, driveChainTemplate, Condition.UNKNOWN, 0));
          newMotorcycleComponents.add(
              new Component(vehicle, chainLubeTemplate, Condition.UNKNOWN, 0));
          newMotorcycleComponents.add(
              new Component(vehicle, sprocketsTemplate, Condition.UNKNOWN, 0));
        } else if (oldDrivenType == DrivenType.CHAIN_DRIVEN && dt == DrivenType.BELT_DRIVEN) {
          currentMotorcycleComponents.stream()
              .filter(
                  c ->
                      c.getTemplate().getComponentType().equals(ComponentType.DRIVE_CHAIN)
                          || c.getTemplate()
                              .getComponentType()
                              .equals(ComponentType.CHAIN_LUBRICATION)
                          || c.getTemplate().getComponentType().equals(ComponentType.SPROCKETS))
              .forEach(component -> componentService.deleteComponent(component.getId()));

          ComponentTemplate driveBeltTemplate =
              componentTemplateService
                  .getTemplatesForVehicleTypeAndComponentType(
                      VehicleType.MOTORCYCLE, ComponentType.DRIVE_BELT)
                  .get(0);

          newMotorcycleComponents.add(
              new Component(vehicle, driveBeltTemplate, Condition.UNKNOWN, 0));
        }
      }
  ...
  @Override
  public void insertDistanceOrFuelForVehicle(
      Long vehicleId, String unitType, Integer amount, LocalDateTime insertTime, boolean late)
      throws IntervalDoesNotMatchException, InsertDateTooLateException {
    Vehicle vehicle = this.findById(vehicleId);
    IntervalUnit unit = IntervalUnit.valueOf(unitType);
    List<Component> components =
        componentService.findAllByVehicleAndTemplateMeasuringUnitType(vehicle, unitType);
    ...
    long needsCheckCount = components.stream().filter(Component::isNeedsCheck).count();

    vehicle.setNeedsCheck(
        totalComponents > 0 && ((double) needsCheckCount / totalComponents) >= 0.3);
  ...
  public void changeVehicleCondition(Long id) {
    ...
    Map<Condition, Long> count =
        components.stream()
            .collect(Collectors.groupingBy(Component::getCondition, Collectors.counting()));
