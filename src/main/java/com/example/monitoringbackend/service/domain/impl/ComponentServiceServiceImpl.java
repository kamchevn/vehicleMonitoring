...
public class ComponentServiceServiceImpl implements ComponentServiceService {
  ...
  public ComponentServiceServiceImpl(
  ...
  public ComponentService findById(Long checkId) {
    return componentServiceRepository
        .findById(checkId)
        .orElseThrow(() -> new ServiceNotFoundException(checkId));
  }

  @Override
  public Page<ComponentService> findPage(
      UserDetails user, Long vehicleId, String serviceType, Integer pageNum, Integer pageSize) {
    ...
    if (!isAdmin) {
      spec =
          spec.and(
              filterEquals(ComponentService.class, "vehicle.user.username", user.getUsername()));
    }
    ...
    return this.componentServiceRepository.findAll(spec, PageRequest.of(pageNum, pageSize));
  }

  @Override
  public void checkConditionForVehicle(
      Long vehicleId,
      String serviceType,
      String note,
