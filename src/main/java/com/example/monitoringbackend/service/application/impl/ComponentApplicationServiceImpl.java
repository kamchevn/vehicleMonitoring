...
public class ComponentApplicationServiceImpl implements ComponentApplicationService {
    ...
    @Override
    public Page<DisplayComponentDto> findPage(Long vehicleId, String measuringUnit, List<String> conditions, Integer pageNum, Integer pageSize) {
        Page<Component> page = componentService.findPage(vehicleId,measuringUnit,conditions,pageNum,pageSize);
    ...
    @Override
    public DisplayComponentDto createNewComponent(Vehicle vehicle, Long componentTemplateId, String condition, Integer counter) {
        return DisplayComponentDto.from(componentService.createNewComponent(vehicle,componentTemplateId,condition,counter));
    }

    @Override
    public DisplayComponentDto editComponent(Long id, Vehicle vehicle, Long componentTemplateId, String condition, Integer counter) {
        return DisplayComponentDto.from(componentService.editComponent(id,vehicle,componentTemplateId,condition,counter));
    }
    ...
    @Override
    public List<DisplayComponentDto> findAllByVehicleAndTemplateMeasuringUnitType(Vehicle vehicle, String measuringUnitType) {
        List<Component> components = componentService.findAllByVehicleAndTemplateMeasuringUnitType(vehicle, measuringUnitType);
        return components.stream().map(DisplayComponentDto::from).toList();
    }
    ...
    @Override
    public List<DisplayComponentDto> findAllByVehicleAndCondition(Vehicle vehicle, Condition condition) {
